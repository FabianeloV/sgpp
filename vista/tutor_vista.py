...
class TutorEmpresarialVista(SeccionBase):
    def __init__(self, ctrl, parent=None):
        self.ctrl = ctrl
        super().__init__("🤝  Tutores Empresariales", parent)
    ...
    def _cargar_filas(self):
        return [
            ...
            for t in self.ctrl.listar_empresariales()
        ]

    def _on_agregar(self):
        dlg = DialogoTutorEmpresarial(parent=self)
        if dlg.exec() == DialogoBase.DialogCode.Accepted:
    ...
    def _on_editar(self, id_):
        ...
        dlg = DialogoTutorEmpresarial(tutor=t, parent=self)
        if dlg.exec() == DialogoBase.DialogCode.Accepted:
